...
from agents.agents.copilot import CopilotAgent
from agents.schemas.output import CopilotInput, CopilotOutput, OrgContext
from agents.schemas.question import Question, SkipLogicRule, SkipLogicCondition, DisplayLogic
...
async def test_copilot_refine_question_text(agent, questions, org_context):
    ...
    with patch("agents.agents.copilot.call_agent", new=AsyncMock(return_value=(output, credit))):
...
async def test_copilot_passes_org_context_to_llm(agent, questions):
    ...
    with patch("agents.agents.copilot.call_agent", new=capture_call_agent):
        await agent.run(inp)
