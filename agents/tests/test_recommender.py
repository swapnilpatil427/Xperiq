...
from agents.agents.recommender import RecommenderAgent
from agents.schemas.output import RecommenderInput, RecommenderOutput, Recommendation, OrgContext
...
async def test_recommender_high_qc_can_distribute(agent, high_qc_input):
    ...
    with patch("agents.agents.recommender.call_agent", new=AsyncMock(return_value=(output, credit))):
...
async def test_recommender_low_qc_cannot_distribute(agent, low_qc_input):
    ...
    with patch("agents.agents.recommender.call_agent", new=capture):
...
async def test_recommender_post_processing_strips_distribute_on_low_qc(agent, low_qc_input):
    ...
    with patch("agents.agents.recommender.call_agent", new=AsyncMock(return_value=(cheating_output, credit))):
        result, _ = await agent.run(low_qc_input)
...
async def test_recommender_post_processing_skips_session_actions(agent, high_qc_input):
    ...
    from agents.schemas.output import SessionAction
