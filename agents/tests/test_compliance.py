...
from agents.agents.compliance import ComplianceAgent
from agents.schemas.output import ComplianceFinding, ComplianceInput, ComplianceOutput, OrgContext
...
async def test_compliance_clean_survey(agent, clean_survey):
    ...
    with patch("agents.agents.compliance.call_agent", new=AsyncMock(return_value=(clean_output, credit))):
...
async def test_compliance_pii_survey_high_risk(agent, pii_survey):
    ...
    with patch("agents.agents.compliance.call_agent", new=AsyncMock(return_value=(high_risk_output, credit))):
...
async def test_compliance_enforces_blocks_distribution_invariant(agent, pii_survey):
    ...
    with patch("agents.agents.compliance.call_agent", new=AsyncMock(return_value=(buggy_output, credit))):
...
async def test_compliance_pii_patterns_injected_into_prompt(agent, pii_survey):
    ...
    with patch("agents.agents.compliance.call_agent", new=capture):
