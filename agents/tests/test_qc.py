...
from agents.agents.qc import QualityControlAgent
from agents.schemas.output import QCInput, QCOutput, QCIssue, QCValidationOutput, OrgContext
...
async def test_qc_clean_survey_passes(agent, clean_survey_input):
    ...
    with patch("agents.agents.qc.call_agent", new=AsyncMock(return_value=(clean_output, credit))):
...
async def test_qc_biased_survey_flags_issues(agent, biased_survey_input):
    ...
    with patch("agents.agents.qc.call_agent", new=AsyncMock(return_value=(biased_output, credit))):
...
async def test_qc_haiku_validator_agrees(agent, clean_survey_input):
    ...
    with patch("agents.agents.qc.call_agent", new=side_effect):
        output, credits = await agent.run(clean_survey_input)
