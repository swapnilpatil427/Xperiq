...
from agents.lib.models import ModelConfig
from agents.lib.openrouter import (
    OpenRouterError,
    _retry_wait,
    _retry_loop,
    _call_with_backoff,
...
class TestRetryLoop:
    ...
    async def test_returns_on_first_success(self, model_cfg):
        with patch("agents.lib.openrouter._raw_call", new_callable=AsyncMock) as mock:
    ...
    async def test_retries_on_429_then_succeeds(self, model_cfg):
        ...
        with patch("agents.lib.openrouter._raw_call", side_effect=side_effect):
            with patch("agents.lib.openrouter.asyncio.sleep", new_callable=AsyncMock):
...
class TestCircuitBreakerIntegration:
    ...
    async def test_circuit_does_not_open_from_single_exhausted_sequence(self, model_cfg):
        ...
        with patch("agents.lib.openrouter.openrouter_breaker", breaker):
            with patch("agents.lib.openrouter._raw_call", new_callable=AsyncMock) as mock:
                mock.side_effect = _rate_limit_err(retry_after=0.001)
                with patch("agents.lib.openrouter.asyncio.sleep", new_callable=AsyncMock):
        ...
        from agents.lib.circuit_breaker import CBState
