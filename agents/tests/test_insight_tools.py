    ...
    def test_headline_max_length(self):
        with pytest.raises(Exception):
            NarrateInsightOutput(
                headline="x" * 121,  # exceeds max_length=120
...
async def test_node_metrics_computes_nps():
    from agents.graphs.insights import node_metrics
    state = {
    ...
    with patch("agents.graphs.insights._emit_event", new_callable=AsyncMock):
...
async def test_node_narrate_uses_call_agent():
    from agents.graphs.insights import node_narrate
    ...
    with patch("agents.graphs.insights._narrate", new_callable=AsyncMock, return_value=mock_narrate_output), \
         patch("agents.graphs.insights._emit_event", new_callable=AsyncMock):
...
async def test_node_verify_demotes_unsupported():
    from agents.graphs.insights import node_verify
    from agents.schemas.insight import VerifyInsightOutput
    ...
    with patch("agents.graphs.insights._verify", new_callable=AsyncMock, return_value=not_supported), \
         patch("agents.graphs.insights._emit_event", new_callable=AsyncMock):
...
class TestEmbeddingsHeuristicFallback:
    ...
    def test_bow_returns_list_of_lists(self):
        from agents.tools.embeddings import _build_bow_embeddings
    ...
    async def test_embed_texts_fallback_no_key(self):
        ...
        import os
        from agents.tools.embeddings import embed_texts
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            try:
                ...
                with patch("agents.tools.embeddings._OPENAI_API_KEY", ""):
...
class TestTopicFuzzyMatching:
    def test_exact_match(self):
        from agents.tools.topics import _fuzzy_matches_any
    ...
    def test_levenshtein_distance_basic(self):
        from agents.tools.topics import _levenshtein
...
class TestDynamicTrustScores:
    def test_trust_statistical_large_n(self):
        from agents.graphs.insights import _trust_statistical
    ...
    def test_trust_coverage_full(self):
        from agents.graphs.insights import _trust_coverage
    ...
    def test_trust_consistency_uniform_cluster(self):
        from agents.graphs.insights import _trust_consistency
        cluster = {
    ...
    def test_build_trust_returns_tuple(self):
        from agents.graphs.insights import _build_trust
