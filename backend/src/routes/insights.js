...
router.get('/:surveyId/run-status', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      // Failure reason — last entry in error_log when status=failed
      error:            errorLog.length ? errorLog[errorLog.length - 1] : null,
      error_log:        errorLog,
      duration_seconds: run.duration_seconds != null ? parseInt(run.duration_seconds) : null,
...
// Returns AI-generated recommended next actions for a survey.
// Generated by the action-recommender XOS skill after each pipeline run.

router.get('/:surveyId/actions', requireAuth, async (req, res) => {
  const { surveyId } = req.params;
  try {
    const { rows } = await db.query(
    ...
    if (!rows.length) {
      return res.json({ actions: [], urgency_level: null, summary: null, generated_at: null });
    }

    const { actions_json, urgency_level, summary, generated_at, dismissed_ids } = rows[0];
    const dismissedSet = new Set(dismissed_ids || []);
    const actions = (actions_json || []).filter(a => !dismissedSet.has(a.id));

    res.json({ actions, urgency_level, summary, generated_at });
  } catch (err) {
    logger.error({ err: err.message, surveyId }, 'insights:actions:error');
    res.json({ actions: [], urgency_level: null, summary: null });
  }
});

// ── POST /api/insights/:surveyId/actions/:actionId/dismiss ────────────────────
// Mark an action as dismissed so it doesn't show again.

router.post('/:surveyId/actions/:actionId/dismiss', requireAuth, async (req, res) => {
  ...
  try {
    ...
    logger.error({ err: err.message, surveyId, actionId }, 'insights:actions:dismiss:error');
