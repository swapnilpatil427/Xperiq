...
export const OPERATORS = new Set(['eq', 'neq', 'contains', 'starts_with', 'gt', 'lt', 'in', 'not_in']);
 ...
 */
export function buildDynamicGroupSQL(ruleSet: DynamicRuleSet | null | undefined, orgId: string): SqlParams {
  const operator = ((ruleSet?.operator || 'AND') as string).toUpperCase();
...
export async function evaluateDynamicGroup(groupId: string, orgId: string): Promise<DiffResult> {
  const { rows: [group] } = await query(
    'SELECT group_type, dynamic_rules FROM user_groups WHERE id = $1 AND org_id = $2',
    [groupId, orgId]
  );
  const g = group as { group_type?: string; dynamic_rules?: DynamicRuleSet } | undefined;
  if (!g || g.group_type !== 'dynamic' || !g.dynamic_rules) return { added: 0, removed: 0 };

  const { sql, params } = buildDynamicGroupSQL(g.dynamic_rules, orgId);
  const { rows: matching } = await query(sql, params);
  const matchingIds = new Set((matching as Array<{ user_id: string }>).map((r) => r.user_id));

  const { rows: current } = await query(
    'SELECT user_id FROM user_group_members WHERE group_id = $1', [groupId]
  );
  const currentIds = new Set((current as Array<{ user_id: string }>).map((m) => m.user_id));
...
export async function evaluateDynamicGroupsForOrg(orgId: string): Promise<number> {
  ...
  for (const { id } of rows as Array<{ id: string }>) {
    try { await evaluateDynamicGroup(id, orgId); }
      ...
      try {
        ...
        (require('./logger') as { warn: (obj: unknown) => void }).warn({ event: 'dynamic_group_eval_failed', groupId: id, err: msg });
