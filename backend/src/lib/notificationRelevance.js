...
const PRIORITY_WEIGHT = { critical: 1.0, warning: 0.8, success: 0.55, info: 0.4, digest: 0.2 };
const SUPPRESS_THRESHOLD = 0.4;

/**
 * Relevance score 0..1.
 * @param {object} f
 * @param {string} f.priority
 * @param {number} [f.magnitude]   normalized 0..1 size of the change (e.g. |delta|/scale)
 * @param {number} [f.recencyHours] hours since the related event (fresher = higher)
 */
function relevanceScore({ priority = 'info', magnitude = 0.5, recencyHours = 0 }) {
 ...
 * @param {object} n  { priority, magnitude?, recencyHours?, unreadSameEntityInfo? }
 */
function shouldSuppress(n) {
...
module.exports = { relevanceScore, shouldSuppress, SUPPRESS_THRESHOLD, PRIORITY_WEIGHT };
