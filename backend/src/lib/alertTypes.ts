...
export function categoryOf(code: string): string { return code.split('-')[0]; }

export interface CatalogEntry extends AlertTypeDef {
  category: string;
  categoryName: string;
}
...
export const VALID_CODES = new Set(ALERT_TYPES.map((t) => t.code));
