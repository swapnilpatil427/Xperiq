...
from crystalos.schemas.question import Question, SkipLogicRule, SkipLogicCondition
...
async def test_add_question_appends_to_end():
    ...
    with patch("crystalos.main.db.get_run_by_id", new=AsyncMock(return_value=_mock_row())):
        with patch("crystalos.main.db.save_run_questions", new=AsyncMock()) as save_mock:
...
async def test_add_question_inserts_after_specified_id():
    ...
    with patch("crystalos.main.db.get_run_by_id", new=AsyncMock(return_value=_mock_row())):
        with patch("crystalos.main.db.save_run_questions", new=AsyncMock()):
            result = await add_question(run_id="run-1", body=body, _key=None)
...
async def test_add_question_assigns_unique_id():
    ...
    with patch("crystalos.main.db.get_run_by_id", new=AsyncMock(return_value=_mock_row(sparse))):
        with patch("crystalos.main.db.save_run_questions", new=AsyncMock()):
            result = await add_question(run_id="run-1", body=body, _key=None)
...
async def test_add_question_404_on_missing_run():
    body = AddQuestionRequest(org_id="org-1", type="open_text")
    with patch("crystalos.main.db.get_run_by_id", new=AsyncMock(return_value=None)):
...
async def test_remove_question_cleans_skip_rules_referencing_it():
    ...
    from crystalos.schemas.question import SkipLogicRule, SkipLogicCondition
    ...
    request = MagicMock()
    request.query_params = {"org_id": "org-1"}
    with patch("crystalos.main.db.get_run_by_id", new=AsyncMock(return_value=_mock_row(qs_as_dicts))):
        with patch("crystalos.main.db.save_run_questions", new=AsyncMock()):
            result = await remove_question(run_id="run-1", q_id="q3", request=request, _key=None)
...
async def test_dispatch_skip_logic_action_calls_agent():
    ...
    with patch("crystalos.main.skip_logic_agent.run", new=AsyncMock(return_value=(mock_output, []))):
        updated, msg = await _dispatch_recommendation(
...
async def test_dispatch_refine_with_question_id_calls_refiner():
    qs = [Question.model_validate(q) for q in SAMPLE_QUESTIONS]
    from crystalos.schemas.output import RefinerOutput
    ...
    with patch("crystalos.main.refiner_agent.run", new=AsyncMock(return_value=(mock_output, []))):
        updated, msg = await _dispatch_recommendation(
...
async def test_dispatch_add_followup_calls_copilot():
    ...
    with patch("crystalos.main.copilot_agent.run", new=AsyncMock(return_value=(mock_output, []))):
        updated, msg = await _dispatch_recommendation(
