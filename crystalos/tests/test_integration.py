...
_PATCHES = {
    # Trace / events
    "crystalos.lib.trace_context.set_trace_context": MagicMock(),
    # DB high-level helpers
    "crystalos.lib.db.update_run":          AsyncMock(return_value=None),
    "crystalos.lib.db.check_survey_access": AsyncMock(return_value=True),
    # Heartbeat
    "crystalos.graphs.insights._update_heartbeat": AsyncMock(return_value=None),
    # Event publisher
    "crystalos.lib.event_publisher.publish_run_event": AsyncMock(return_value=None),
    # AI / embedding
    "crystalos.lib.openrouter.call_agent":                  AsyncMock(return_value="[]"),
    "crystalos.tools.embeddings.get_or_create_embeddings":  AsyncMock(return_value=[]),
    "crystalos.tools.clustering.cluster_texts":             AsyncMock(return_value=[]),
    "crystalos.tools.sentiment.run_absa_llm":               AsyncMock(return_value=[]),
    # Topic signals / registry
    "crystalos.lib.topic_signals.compute_full_topic_signals": AsyncMock(return_value={}),
    "crystalos.lib.topic_registry.get_centroids":             AsyncMock(return_value=[]),
    "crystalos.lib.topic_registry.has_centroids":             AsyncMock(return_value=False),
    # Checkpoint — patch at the import site in insights.py
    "crystalos.graphs.insights.write_checkpoint_blob": AsyncMock(return_value=None),
    # Context nodes (pass state through unchanged)
    "crystalos.graphs.nodes.context.node_context": AsyncMock(
        side_effect=lambda s: {**s, "org_context": {}, "survey_context": {}}
    ),
    "crystalos.graphs.nodes.context.node_route_specialists": AsyncMock(
...
def _start_patches(overrides=None):
    ...
    pool_patch = patch("crystalos.lib.db._pool_conn", return_value=_make_pool_mock())
    pool_patch.start()
...
class TestPipelineIntegration:
    ...
    async def test_pipeline_returns_dict(self):
        ...
        active = _start_patches({"crystalos.lib.db.check_survey_access": AsyncMock(return_value=False)})
        try:
