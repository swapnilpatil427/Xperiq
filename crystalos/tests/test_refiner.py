...
from crystalos.agents.refiner import RefinerAgent
from crystalos.schemas.output import RefinerInput, RefinerOutput, OrgContext
...
async def test_refiner_removes_bias(agent, original_question, surrounding_questions):
    ...
    with patch("crystalos.agents.refiner.call_agent", new=AsyncMock(return_value=(output, credit))):
...
async def test_refiner_rejects_type_change_without_request(agent, original_question):
    ...
    with patch("crystalos.agents.refiner.call_agent", new=AsyncMock(return_value=(bad_output, credit))):
