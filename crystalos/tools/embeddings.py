...
from crystalos.lib.topic_registry import _format_vector
...
async def embed_text(text: str, org_id: str = "", survey_id: str = "") -> list[float] | None:
    """Embed a single string. Returns the embedding vector or None on failure."""
    try:
        vectors = await embed_texts([text], org_id=org_id, survey_id=survey_id)
