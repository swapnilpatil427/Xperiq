# XOS Skills — Format Specification
...
A skill is the complete definition of one AI capability. It lives in the `agents/skills/` directory. One folder = one skill.
...
You are the Insight Narrator for Experient XOS. Your job is to turn a set of
...
```bash
python -m agents.skills.generate_examples insight-narrator > agents/skills/insight-narrator/EXAMPLES.md
```
...
1. Scanning `agents/skills/*/SKILL.md` at startup
...
7. Add an entry to `agents/tests/skills/test_<skill-name>.py`
