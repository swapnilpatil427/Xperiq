...
function NpsQuestion({ q, value, onChange }) {
...
function CsatQuestion({ q, value, onChange }) {
    ...
    <div className="flex gap-2 mt-4">
      {Array.from({ length: max }, (_, i) => (
        <Button key={i + 1} onClick={() => onChange(i + 1)} variant="secondary"
          ...
          style={{
            background: value >= i + 1 ? '#d97706' : '#eef1f3',
            color: value >= i + 1 ? '#fff' : '#c4c4c4',
          }}>
          {style === 'stars' ? '★' : i + 1}
        </Button>
      ))}
...
function RatingQuestion({ q, value, onChange }) {
    ...
    <div className="flex gap-2 mt-4">
      {Array.from({ length: max }, (_, i) => (
        <Button key={i + 1} onClick={() => onChange(i + 1)} variant="secondary"
          className="flex-1 py-4 rounded-xl font-black text-xl transition-all active:scale-95 border-0"
          style={{
            background: value >= i + 1 ? '#2a4bd9' : '#eef1f3',
            color: value >= i + 1 ? '#ffffff' : '#c4c4c4',
            boxShadow: value >= i + 1 ? '0 8px 20px rgba(42,75,217,0.2)' : 'none',
          }}>
...
function OpenTextQuestion({ q, value, onChange }) {
  const { t } = useTranslation();
  return (
    <Textarea
      value={value || ''}
...
function ShortTextQuestion({ q, value, onChange }) {
  return (
    <input
      type="text"
      value={value || ''}
...
function MatrixQuestion({ question, value = {}, onChange }) {
  ...
  const isSelected = (row, col) => multi ? (value[row] || []).includes(col) : value[row] === col;
  return (
...
function DateQuestion({ q, value, onChange }) {
...
function SurveyErrorScreen({ type, onRetry }) {
  const { t } = useTranslation();
  const SURVEY_ERROR_CONFIG = {
  ...
  const keyMap = { not_found: 'notFound', closed: 'closed', paused: 'paused', network: 'network' };
...
export function SurveyFillPage() {
  ...
  const [retryAfterSecs, setRetryAfterSecs] = useState(null);
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div
          ...
          style={{ top: '-15%', left: '-10%', width: '40%', height: '40%', background: 'rgba(42,75,217,0.12)', filter: 'blur(120px)' }}
        />
        <div
          className="absolute rounded-full"
          style={{ bottom: '-10%', right: '-10%', width: '50%', height: '50%', background: 'rgba(131,41,200,0.12)', filter: 'blur(150px)' }}
        />
      </div>
        ...
        <div className="w-full max-w-xl pt-8">
          ...
          {q && (
            ...
            >
              ...
              {q.type === 'nps' && <NpsQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'csat' && <CsatQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'rating' && <RatingQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'multiple_choice' && <MultipleChoiceQuestion question={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              ...
              {q.type === 'open_text' && <OpenTextQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'short_text' && <ShortTextQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'slider' && <SliderQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'ranking' && <RankingQuestion question={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'matrix' && <MatrixQuestion question={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'date' && <DateQuestion q={q} value={answers[q.id]} onChange={(v) => setAnswer(q.id, v)} />}
              {q.type === 'statement' && <StatementBlock q={q} />}
