...
/** SurveyResponse augmented with the parent survey for display purposes. */
...
const fadeUp = {
  ...
  visible: (i = 0) => ({ opacity: 1, y: 0, transition: { duration: 0.35, delay: i * 0.04, ease: [0.22, 1, 0.36, 1] as [number, number, number, number] } }),
};
  ...
  const loadResponses = useCallback(async () => {
    ...
    try {
      if (selectedSurveyId === 'all') {
        ...
        const r = await api.listSurveys({ status: ['active' as SurveyStatus], limit: 10 });
        const activeSurveys = r.surveys || [];
        const allResponses: ResponseRow[] = [];
        await Promise.all(
          activeSurveys.slice(0, 5).map(async (survey: Survey) => {
            try {
              const res = await api.getResponses(survey.id);
              const rows: ResponseRow[] = (res.responses || []).map((resp: SurveyResponse) => ({ ...resp, _survey: survey }));
              allResponses.push(...rows);
            } catch {}
          })
        );
        allResponses.sort((a, b) => new Date(b.submitted_at).getTime() - new Date(a.submitted_at).getTime());
        setResponses(allResponses.slice(0, 50));
      } else {
        const res = await api.getResponses(selectedSurveyId);
        const survey = surveys.find((s: Survey) => s.id === selectedSurveyId);
        const rows: ResponseRow[] = (res.responses || []).map((resp: SurveyResponse) => ({ ...resp, _survey: survey }));
        setResponses(rows);
      }
    } catch (err) {
      ...
      {!loading && !error && responses.length > 0 && (
        ...
        >
          {responses.map((resp, i) => {
            ...
            const accentColor = (SURVEY_TYPE_COLORS as Record<string, string>)[survey?.survey_type_id ?? ''] || '#6b7280';
                ...
                <div className="flex items-start gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className="text-xs text-on-surface-variant">{timeAgo(resp.submitted_at)}</span>
                    </div>
