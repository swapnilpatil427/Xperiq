...
import type { InsightTopic } from '../types';
...
function sentimentToSignal(sentiment: InsightTopic['sentiment'], translate: typeof tFn): { signal: string; signalColor: string } {
  if (sentiment === 'positive') return { signal: translate('insights.signals.strongPositive'), signalColor: '#059669' };
  if (sentiment === 'negative') return { signal: translate('insights.signals.critical'),       signalColor: '#b41340' };
  return                               { signal: translate('insights.signals.neutral'),         signalColor: '#d97706' };
}
  ...
  return (
        <div className="space-y-8 max-w-7xl mx-auto w-full">
          ...
          <PageHeader
            ...
            subtitle={t('insights.topicDescription', { count: topics.length })}
          />
          ...
          <section className="space-y-6">
            ...
            >
              <div>
                ...
                <p className="text-sm text-on-surface-variant">
                  {t('insights.topicDescription', { count: (insights?.response_count ?? 12482).toLocaleString() })}
                </p>
              </div>
            ...
            <motion.div
              ...
              animate="visible"
            >
              {((insights?.topics || topics) as (InsightTopic | StaticTopic)[]).map((topic, idx) => {
                const isRaw = topic.name !== undefined && !('title' in topic);
                const rawTopic = isRaw ? (topic as InsightTopic) : null;
                const staticTopic = !isRaw ? (topic as StaticTopic) : null;
                const title = rawTopic ? rawTopic.name : staticTopic!.title;
                const volume = rawTopic ? `${rawTopic.volume.toLocaleString()} VOL` : staticTopic!.volume;
                const isActive = rawTopic ? rawTopic.sentiment === 'negative' : staticTopic!.active;
                const iconName = rawTopic ? TOPIC_ICONS[idx % TOPIC_ICONS.length] : staticTopic!.icon;
                const sigInfo = rawTopic ? sentimentToSignal(rawTopic.sentiment, t) : { signal: staticTopic!.signal, signalColor: staticTopic!.signalColor };
                const signalDot = rawTopic ? (rawTopic.sentiment === 'positive' ? '#10b981' : rawTopic.sentiment === 'negative' ? '#b41340' : '#f59e0b') : staticTopic!.signalDot;
                return (
                  <motion.button
                    key={rawTopic ? rawTopic.name : staticTopic!.id}
                    custom={idx}
                    variants={fadeUp}
                    ...
                    <div className="flex flex-col md:flex-row gap-16">
                      ...
                      <div className="flex-1 space-y-6">
                        ...
                        <div className="flex items-end justify-between h-48 gap-6">
                          {(() => {
                            const bd = insights?.sentiment_breakdown || SENTIMENT.DEFAULT;
                            return [
                      ...
                      <div className="flex-1 space-y-6">
                        ...
                        <div className="space-y-3">
                          {(insights?.top_phrases || frictionPoints.map((f) => f.phrase)).map((phrase, i) => (
                            <motion.div
                              key={i}
