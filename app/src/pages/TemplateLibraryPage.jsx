...
function DeleteModal({ template, onDelete, onClose }) {
  const { t } = useTranslation();
...
function BenchmarkBar({ benchmarks, unit, t }) {
...
function InfoModal({ template, onClose, onUseClick }) {
  const { t } = useTranslation();
  const cat = SURVEY_CATEGORIES.find((c) => c.id === template.category);
  const intel = template.intelligence || {};
  const scoring = template.scoring || {};
  const scoringLabel = scoring.method ? (SCORING_METHOD_LABELS[scoring.method] || scoring.method) : null;
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-2xl">
      ...
      <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
        ...
        {/* Benchmarks */}
        {scoring.benchmarks && Object.keys(scoring.benchmarks).length > 0 && (
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-on-surface-variant mb-3">
              {t('templates.infoModal.benchmarks')}
            </h3>
            <BenchmarkBar benchmarks={scoring.benchmarks} unit={scoring.unit} t={t} />
          </div>
        )}
            ...
            <div className="space-y-3">
              {intel.scoringNarrative && (
                <div>
                  ...
                  <p className="text-xs text-on-surface leading-relaxed">{intel.scoringNarrative}</p>
                </div>
              )}
              {intel.audienceDescription && (
                <div>
                  ...
                  <p className="text-xs text-on-surface leading-relaxed">{intel.audienceDescription}</p>
                </div>
              )}
            ...
            <div className="space-y-2">
              {sampleQuestions.map((q, i) => (
                <div key={q.id || i} className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl"
                  ...
                  <span className="text-xs text-on-surface leading-relaxed">{q.text || q.label || q.question}</span>
                </div>
              ))}
            </div>
          </div>
...
function TemplateModal({ template, categories, onSave, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    tags:        template?.tags        || [],
    intelligence: {
      scoringNarrative:    template?.intelligence?.scoringNarrative    || '',
      audienceDescription: template?.intelligence?.audienceDescription || '',
      customInstructions:  template?.intelligence?.customInstructions  || '',
    },
  });
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSaveError(err.message || t('templates.modal.errors.saveFailed'));
      setSaving(false);
    }
  }
...
function TemplateCard({ template, onUseClick, onEdit, onDelete, onInfoClick }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
...
export function TemplateLibraryPage() {
  ...
  // Modal states
  const [editModal, setEditModal] = useState(null);    // null | template (create: null obj, edit: template)
  const [useModal, setUseModal] = useState(null);      // null | template
  const [deleteModal, setDeleteModal] = useState(null); // null | template
  const [infoModal, setInfoModal] = useState(null);     // null | template
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      ...
      setError(t('templates.errorLoading', { message: err.message }));
    } finally {
      setLoading(false);
    }
  ...
  async function handleSave(formData) {
    if (formData.id && !formData.isSystem) {
      ...
      setTemplates((prev) => prev.map((tmpl) => (tmpl.id === formData.id ? { ...tmpl, ...formData } : tmpl)));
    } else {
