...
export interface IrisChatProps {
  context?: IrisChatContext;
...
function buildGreeting({ surveyType, questionCount }: IrisChatContext): string {
  if (questionCount) {
...
export function IrisChat({ context = {}, onRefine }: IrisChatProps) {
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const reply: ChatMessage = { role: 'ai', text: result.explanation || 'Done! Survey updated.' };
