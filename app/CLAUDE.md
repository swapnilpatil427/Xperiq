...
- **AppShell pattern** — `components/AppShell.jsx` wraps all authenticated pages (SideNav + TopBar + BottomNav). SurveyBuilderPage is inside AppShell using `isBuilder` mode — gutters/footer/BottomNav are skipped and the page renders fixed panels directly.
